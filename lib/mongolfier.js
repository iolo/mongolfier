...
function processCollection(mysqlConn, mongoConn, config, collectionConf, context) {
  ...
  return loadScript(collectionConf.template || (collectionConf.collection + '.json'))
    .fail(function (err) {
      DEBUG && debug('load template err', err);
      if (config.bulkCopy) {
        DEBUG && debug('no template! do bulkcopy for collection:', collectionName);
        return null;
      }
      DEBUG && debug('no template! skip collection:', collectionName);
      throw err;
    })
    .then(function (template) {
      return [openMongoCollection(mongoConn, collectionName, config.emptyCollection), template];
    })

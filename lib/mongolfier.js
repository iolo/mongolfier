...
function loadScript(filePath) {
  ...
  try {
    ...
    logError('failed to load script:'+filePath, e);
    return null;
  }
}
...
function processHook(hook) {
  ...
  try {
    ...
    logError('failed to execute hook:'+hook, e);
    d.reject(e);
  }
  return d.promise;
}
...
function executeMysqlQuery(query, params) {
  ...
  mysqlConn.query(query, params, function(err, rows) {
    if (err) {
      logError('failed to execute mysql query!', err);
      d.reject(err);
      return;
    }
...
function openMongoConn(mongoConf) {
  ...
  mongoConn.open(function (err) {
    if (err) {
      logError('failed to open mongo connection!', err);
      d.reject(err);
      return;
    }
    ...
    if (!!mongoConf.user && !!mongoConf.password) {
      mongoConn.authenticate(mongoConf.user, mongoConf.password, function(err) {
        if (err) {
          logError('failed to authenticate mongo connection!', err);
          d.reject(err);
          return;
        }
...
function openMongoCollection(collectionName) {
  ...
  mongoConn.collection(collectionName, function(err, mongoCollection) {
    if (err) {
      logError('failed to open mongo collection!', err);
      d.reject(err);
      return;
    }
    ...
    if (config.emptycollection) {
      ...
      mongoCollection.remove({}, function (err, removed) {
        if (err) {
          logError('failed to empty mongo collection!', err);
          d.reject(err);
          return;
        }
...
function insertMongoDoc(mongoDoc, mongoCollection) {
  ...
  if (config.dryrun) {
    ...
    mongoCollection.insert(mongoDoc, function (err, result) {
      if (err) {
        logError('failed to insert mongo document!', err);
        d.reject(err);
        return;
      }
...
function processRow(templateScript, mongoCollection, row) {
  ...
  try {
    if (templateScript) {
      ...
      q.fcall(function () {
        return templateScript.runInNewContext(sandbox);
      })
...
function processCollection(collectionConf, mongoCollection) {
  ...
  .then(function (rows) {
    _.each(rows, function(row, index) {
      processRow(collectionConf.templateScript, mongoCollection, row).then(function () {
        ...
        logError('failed to process row!', err);
        d.reject(err);
      });
    });
  }).fail(function (err) {
  ...
  */
  ...
  queue = async.queue(function(row, callback) {
    processRow(collectionConf.templateScript, mongoCollection, row)
    .then(function (result) {
      rowCount += 1;
      logInfo('------------------------------#', rowCount);
